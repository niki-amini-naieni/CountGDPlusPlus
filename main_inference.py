...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument(
        "--fscd_gt_file", type=str, required=True, help="name of FSCD147 ground truth COCO file"
    )
    parser.add_argument(
        "--coco_output_file", type=str, required=True, help="name of COCO output file for predictions"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sam_tt_norm",
        action="store_true",
        help="apply test-time normalization using Segment Anything Model (SAM) for refinement and visual exemplars as box prompts. Note: the boxes saved in the output COCO file are the original unrefined ones. This operation only improves the count, not the boxes.",
    )
    parser.add_argument("--sam_model_path", default="./checkpoints/sam_vit_h_4b8939.pth", help="path to SAM model checkpoint")
    parser.add_argument(
        "--crop",
        action="store_true",
        help="use real or pseudo visual exemplars to adaptively crop images with max number of objects (i.e., [num_select], e.g., 900, 900-slack) detected into smaller pieces for final count prediction. Super-resolution has been applied to the cropped images.",
    )
    parser.add_argument(
        "--superres_folder",
        type=str, 
