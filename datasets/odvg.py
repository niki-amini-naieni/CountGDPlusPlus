...
class ODVGDataset(VisionDataset):
    ...
    def __getitem__(self, index: int):
        ...
        w, h = image.size
        if self.dataset_mode == "OD":
            # Process the dots for the instances.
            anno = meta["detection"]
            instances = [obj for obj in anno["instances"]]
            exemplar_labels = [str(obj["label"]) for obj in meta["exemplars"]]
            boxes = [obj["bbox"] for obj in instances]
            # generate vg_labels
            # pos bbox labels
            ori_classes = [str(obj["label"]) for obj in instances] + exemplar_labels
            ...
            # Process the exemplars for the instances.
            exemplars = [obj for obj in meta["exemplars"]]
            exemplar_boxes = [obj["bbox"] for obj in exemplars]
            exemplar_boxes = torch.as_tensor(exemplar_boxes, dtype=torch.float32).reshape(-1, 4)
            exemplar_classes = [caption_dict[self.label_map[str(obj["label"])]] for obj in exemplars]
            exemplar_classes = torch.tensor(exemplar_classes, dtype=torch.int64)
        ...
        target["exemplar_labels"] = exemplar_classes
